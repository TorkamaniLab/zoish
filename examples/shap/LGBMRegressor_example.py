...
def test_shap_feature_selector_regression(model, dataset_func):
    ...
    selector = ShapFeatureSelector(model, num_features=int(X_train.shape[1] * 0.5), n_iter=5,
                                   direction="maximum", scoring="neg_mean_squared_error", cv=KFold(n_splits=5, shuffle=True, random_state=RANDOM_SEED))
...
model = LGBMRegressor(random_state=RANDOM_SEED, force_col_wise=True, max_depth=6, num_leaves=31,
                      min_data_in_leaf=20, learning_rate=0.05, min_gain_to_split=0.01, 
                      bagging_fraction=0.8, bagging_freq=5, n_estimators=100)
